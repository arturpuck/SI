   ...
   <div v-on-clickaway="hideMenu" class="multiselect">
      ...
      <div class="multiselect__menu"   v-bind:class="{'multiselect__menu--visible' : menuIsVisible}">
          ...
          <ul  class="multiselect__options">
              <li v-for="(option, key) in totalOptions" v-show="optionShouldBeDisplayed(key)" class="multiselect__option">
                 <labeled-checkbox v-on:click.native="updateModel"  v-model="optionsStates[key]" v-bind:name="key">
                     {{option}}
                </labeled-checkbox>
              </li>
          </ul>
      ...
      <ul class="multiselect__selected-options-list">
          <li v-for="(option, key) in selectedOptions" class="multiselect__selected-option">
              {{option}}
              <button-close v-on:click.native="removeSelectedOption(key)" class="button-close--smaller"></button-close>
          </li>
      </ul>
</div> 
</template>
...
@Component({components : {IconAddPlus, LabeledCheckbox, ButtonClose}, directives : {onClickaway}})
  export default class MultiSelect extends Vue {
      ...
      modelChanged(value : Array<any>, oldValue : Array<any>){
         let innerSelectedOptions:object = {};
          ...
          Object.keys(this.totalOptions).forEach(key => {
              ...
              this.$set(this.optionsStates, key, isSelected);
              if(isSelected){
    ...
    replaceAvailableOptions(options:Array<any>){
        ...
        Object.keys(this.displayedOptions).forEach(key => {
          this.$set(this.optionsStates, key, false );
        });
    }
